...
FileBubble::FileBubble(const QJsonObject& metadata, ChatRole role, bool incoming, QWidget* parent)
    ...
    connect(action_, &QPushButton::clicked, this, [this]() {
        if (incoming_)
            emit downloadRequested(metadata_);
        else
            emit cancelRequested(transferId());
    });
}

QString FileBubble::transferId() const {
    return metadata_["id"].toString();
}
void FileBubble::setTransferId(const QString& id) {
    metadata_["id"] = id;
}
void FileBubble::setProgress(qint64 current, qint64 total) {
    progress_->setValue(total > 0 ? static_cast<int>(current * 100 / total) : 100);
    status_->setText(tr("传输中 %1%").arg(progress_->value()));
}
void FileBubble::setFinished(const QString& localPath) {
    progress_->setValue(100);
    status_->setText(localPath.isEmpty() ? tr("发送完成") : tr("已保存到 %1").arg(localPath));
