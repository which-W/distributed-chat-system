...
ChatDialog::ChatDialog(QWidget* parent)
    : QWidget(parent), ui(new Ui::ChatDialogClass()), _mode(ChatUIMode::ChatMode),
    ...
    QPixmap scaledPixmap = pixmap.scaled(ui->front_bar->size(), Qt::KeepAspectRatio,
    ...
    ui->chat_bar->SetState("normal", "hover", "pressed", "selected_normal", "selected_hover",
                           "selected_pressed");

    ui->contact_bar->SetState("normal", "hover", "pressed", "selected_normal", "selected_hover",
                              "selected_pressed");
    ...
    connect(ui->contact_bar, &StateWidget::clicked, ui->contact_list,
            &ContactUserList::sig_clicked_user);
    // 加载用户列表
    connect(ui->chat_search_list, &ChatUserList::sig_loading_chat_user, this,
    ...
    // 连接加载更多联系人的信号和槽函数
    connect(ui->contact_list, &ContactUserList::sig_loading_contact_user, this,
            &ChatDialog::slot_loading_contact_user);

    // 连接contactlist的add_friend_info信号
    connect(ui->contact_list, &ContactUserList::sig_switch_friend_info_page, this,
            &ChatDialog::slot_switch_friend_info_page);

    // 连接contactlist的sig_switch_apply_friend_page信号
    connect(ui->contact_list, &ContactUserList::sig_switch_apply_friend_page, this,
    ...
    connect(ui->friend_info_page, &FriendInfoPage::sig_jump_chat_item, this,
            &ChatDialog::slot_jump_chat_again);
    // 设置chat_page为主页面
...
void ChatDialog::slot_apply_friend(std::shared_ptr<AddFriendApply> applyinfo) {
    ...
    qDebug() << "receive apply friend info, uid is " << applyinfo->_from_uid << ", name is "
             << applyinfo->_name;
...
void ChatDialog::slot_add_auth_friend(std::shared_ptr<AuthInfo> auth_info) {
    qDebug() << "receive slot_add_auth__friend uid is " << auth_info->_uid << " name is "
             << auth_info->_name << " nick is " << auth_info->_nick;
