...
RegisterDialog::RegisterDialog(QWidget* parent)
    : QDialog(parent), ui(new Ui::RegisterDialogClass()), _counter(5) {
    initHandlers();
    // 页面初始化逻辑
    ui->setupUi(this);
    ...
    toggleAction2 =
        ui->psw_edit_2->addAction(QIcon(":/res/close.png"), QLineEdit::TrailingPosition);
    connect(toggleAction, &QAction::triggered, this, &RegisterDialog::togglePasswordVisibility);
    connect(toggleAction2, &QAction::triggered, this,
            &RegisterDialog::togglePasswordconfirmVisibility);
    ui->err_tip->hide();
    // 消息发送逻辑
    ...
    connect(Httpmgr::Getinstance().get(), &Httpmgr::sig_reg_mod_finish, this,
            &RegisterDialog::slot_req_mod_finished);
    ...
    connect(ui->cansel_btn, &QPushButton::clicked, this,
            &RegisterDialog::on_return_cansel_btn_clicked);
    // 输入框检测逻辑
    ...
    connect(ui->psw_edit, &QLineEdit::editingFinished, this, [this]() { checkPassValid(); });
    connect(ui->psw_edit_2, &QLineEdit::editingFinished, this, [this]() { checkConfirmValid(); });
    connect(ui->code_edit, &QLineEdit::editingFinished, this, [this]() { checkVarifyValid(); });
...
void RegisterDialog::get_code_func() {
    ...
    if (match) {
        QJsonObject json;
        json["email"] = email;
        Httpmgr::Getinstance()->PostHttpRequest(gate_url_prefix + "/post_email", json,
                                                Req::ID_GET_VERIFT_CODE, Modules::MOD_REGISTER);
    } else {
...
void RegisterDialog::showTip(QString str, bool b_ok) {
    if (isVisible()) {
        if (b_ok) {
            ...
            ElaMessageBar::error(ElaMessageBarType::TopRight, tr("请检查输入"), str, 3200,
                                 window());
        }
    }
}
...
void RegisterDialog::slot_reg_finished() {
    ...
    Httpmgr::Getinstance()->PostHttpRequest(gate_url_prefix + "/user_register", json_obj,
                                            Req::ID_REQ_USER, Modules::MOD_REGISTER);
}
