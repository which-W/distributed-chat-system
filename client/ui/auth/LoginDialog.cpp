...
#include <QTimer>
LoginDialog::LoginDialog(QWidget* parent) : QDialog(parent), ui(new Ui::LoginDialogClass()) {
    ui->setupUi(this);
    ui->err_tip->hide();
    ui->psw_line_edit->setEchoMode(QLineEdit::Password);

    toggleAction =
        ui->psw_line_edit->addAction(QIcon(":/res/close.png"), QLineEdit::TrailingPosition);
    ...
    connect(ui->user_line_edit, &QLineEdit::editingFinished, this, [this]() { checkEmailValid(); });
    connect(ui->psw_line_edit, &QLineEdit::editingFinished, this, [this]() { checkPassValid(); });
...
void LoginDialog::updateHeadPixmap() {
    ...
    originalPixmap =
        originalPixmap.scaled(available, Qt::KeepAspectRatio, Qt::SmoothTransformation);
