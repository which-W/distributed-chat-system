...
TcpMgr::TcpMgr()
    ...
    connect(&_socket, &QSslSocket::encrypted, this, [this]() {
        if (_use_tls && !_connect_result_emitted) {
            _connect_result_emitted = true;
            qDebug() << "TLS established with" << _host << "using"
                     << _socket.sessionCipher().name();
            handleTransportReady();
        }
    });
    connect(&_socket, QOverload<const QList<QSslError>&>::of(&QSslSocket::sslErrors), this,
        ...
        forever {
            if (!_b_recv_pending) {
                ...
                if (!_handlers.contains(responseId) || _message_len == 0) {
                    qWarning() << "Rejected invalid chat frame header" << _message_id
                               << _message_len;
                    _socket.abort();
                    return;
                }
...
void TcpMgr::initHandlers() {
    ...
    _handlers.insert(Req::ID_CHAT_LOGIN_RSP, [this](Req id, int len, QByteArray data) {
        ...
        if (err != ErrorCode::ERR_OK) {
            ...
            if (_reconnecting) {
                ...
                emit sig_connection_state(
                    tr("Session re-authentication failed. Please sign in again."), false);
            }
            emit sig_login_failed(err);
            return;
        }
        ...
        const auto scope =
            QCryptographicHash::hash(gate_url_prefix.toUtf8(), QCryptographicHash::Sha256).toHex();
        const auto directory =
            QStandardPaths::writableLocation(QStandardPaths::AppLocalDataLocation) + "/messages/" +
            scope;
        if (!QDir().mkpath(directory) ||
            !_messages.open(directory + '/' + QString::number(uid) + ".sqlite", uid)) {
            emit sig_connection_state(
                tr("Cannot open local message storage. Please check disk space and permissions."),
                false);
            _manual_disconnect = true;
            _socket.abort();
        ...
        for (const auto& value : jsonObj["friend_list"].toArray()) {
            const auto peer = value.toObject()["uid"].toInt();
            std::vector<std::shared_ptr<TextChatData>> history;
            for (const auto& row : _messages.history(peer)) {
                const auto item = row.toObject();
                history.push_back(std::make_shared<TextChatData>(
                    item["msgid"].toString(), item["content"].toString(), item["fromuid"].toInt(),
                    item["touid"].toInt()));
            }
            UserMgr::Getinstance()->AppendFriendChatMsg(peer, history);
        }
    ...
    _handlers.insert(Req::ID_TEXT_CHAT_MSG_RSP, [this](Req id, int len, QByteArray data) {
        ...
        if (!_messages.accept(jsonObj))
            emit sig_connection_state(
                tr("Message acceptance could not be saved locally; it will be retried."), false);
    });
    ...
    _handlers.insert(Req::ID_NOTIFY_TEXT_CHAT_MSG_REQ, [this](Req id, int len, QByteArray data) {
        ...
        if (fromUid <= 0 || toUid != UserMgr::Getinstance()->GetUid() || messages.isEmpty() ||
            messages.size() > 50) {
            _socket.abort();
            return;
        }
        ...
        if (!_messages.save(fromUid, toUid, messages, freshMessages)) {
            emit sig_connection_state(
                tr("Cannot save incoming messages. Delivery will resume after signing in again."),
                false);
            _manual_disconnect = true;
            _socket.abort();
            return;
    ...
    _handlers.insert(Req::ID_NOTIFY_FILE_REQ, [this](Req, int, QByteArray data) {
        ...
        if (document.isObject() && document.object()["error"].toInt() == 0)
            emit sig_file_available(document.object());
    });
}
...
void TcpMgr::beginConnection() {
    ...
    if (_use_tls) {
        if (!QSslSocket::supportsSsl()) {
            qWarning() << "TLS is unavailable in this Qt runtime:"
...
void TcpMgr::slot_send_data(Req reqId, QByteArray data) {
    if (reqId == Req::ID_TEXT_CHAT_MSG_REQ) {
        enqueueText(QJsonDocument::fromJson(data).object());
        return;
    }
...
bool TcpMgr::enqueueText(const QJsonObject& message) {
    for (const auto& value : message["text_array"].toArray()) {
        auto single = message;
        single["text_array"] = QJsonArray{value};
        if (QJsonDocument(single).toJson(QJsonDocument::Compact).size() > 2048) {
            emit sig_connection_state(
                tr("Message is too long. Please split it into shorter messages."), false);
            return false;
        }
    }
    QJsonArray fresh;
    const auto user = UserMgr::Getinstance()->GetUserInfo();
    if (!user || message["fromuid"].toInt() != user->_uid ||
        !_messages.save(message["fromuid"].toInt(), message["touid"].toInt(),
                        message["text_array"].toArray(), fresh)) {
        emit sig_connection_state(
            tr("Message could not be saved. Check its size and available disk space."), false);
        return false;
    }
    flushOutbox();
    return true;
...
void TcpMgr::flushOutbox() {
    if (!_authenticated || _socket.state() != QAbstractSocket::ConnectedState ||
        (_use_tls && !_socket.isEncrypted()))
        return;
    const auto now = QDateTime::currentSecsSinceEpoch();
    for (const auto& value : _messages.pending(now)) {
        if (_socket.bytesToWrite() > 256 * 1024)
            break;
        const auto message = value.toObject();
        if (!_messages.attempted(message, now))
            break;
        writeFrame(Req::ID_TEXT_CHAT_MSG_REQ,
                   QJsonDocument(message).toJson(QJsonDocument::Compact));
    }
}

void TcpMgr::writeFrame(Req reqId, const QByteArray& data) {
    ...
    if (_socket.bytesToWrite() + data.size() + 4 > 4 * 1024 * 1024) {
        emit sig_connection_state(tr("Connection write buffer is full. Please retry later."),
                                  false);
        return;
    }
    uint16_t id = reqId;
