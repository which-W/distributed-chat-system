...
ChatWindow::ChatWindow(QWidget* parent) : ElaWindow(parent) {
    ...
    connect(TcpMgr::Getinstance().get(), &TcpMgr::sig_connection_state, this,
            [this](const QString& message, bool connected) {
                if (connected) {
                    ElaMessageBar::success(ElaMessageBarType::TopRight, tr("Connection restored"),
                                           message, 2800, this);
                } else {
                    ElaMessageBar::warning(ElaMessageBarType::TopRight, tr("Connection status"),
                                           message, 3200, this);
                }
            });
...
QWidget* ChatWindow::createSettingsPage(QWidget* parent) {
    ...
    connect(themeButton, &QPushButton::clicked, &ThemeManager::instance(),
            &ThemeManager::toggleTheme);

    auto* networkTitle = new ElaText(tr("Network route"), page);
    networkTitle->setTextPixelSize(24);
    auto* networkDescription = new ElaText(tr("Keep Direct for the normal entry. Select SOCKS5 "
                                              "backup when v2rayN or Mihomo is running locally."),
                                           page);
    ...
    connect(proxyMode_, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            [this](int) { updateProxyInputs(); });
...
void ChatWindow::applyProxySettings() {
    ...
    QString error;
    if (!ProxyManager::instance().apply(
            mode, proxyHost_->text(), static_cast<quint16>(proxyPort_->text().toUInt()), &error)) {
    ...
    ElaMessageBar::success(ElaMessageBarType::TopRight, tr("Network route applied"),
                           tr("Current mode: %1").arg(ProxyManager::instance().modeName()), 2800,
                           this);
}
...
void ChatWindow::applyTheme(ElaThemeType::ThemeMode mode) {
    ...
    setStyleSheet(
        QString("#navigationRail { background: %1; border: 1px solid %2; border-radius: 16px; }"
                "#contentStack, #settingsPage { background: %3; border: 1px solid %2; "
                "border-radius: 16px; }")
