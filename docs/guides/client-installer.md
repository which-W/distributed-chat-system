...
脚本也会识别本项目 `build/tools/inno/ISCC.exe` 下的打包编译器。
...
## 本次验证记录（2026-09-13，1.0.0）

已在本机独立测试目录验证安装及全部文件哈希、移除 PATH 中 Qt/VS 路径后启动、
运行中阻止安装、同版本重装保留修改后的配置、模拟更高已安装版本时拒绝降级，
以及卸载后删除程序和注册信息并保留配置。示例地址和本地 HTTP 配置也被打包脚本拒绝。
测试安装已卸载。本机有开发环境，因此以上不能替代干净 Windows 测试机验收；
没有使用真实账号验证公网登录、聊天或文件传输，也未验证实际跨版本数据库兼容性。
