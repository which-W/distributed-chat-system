...
inline RedisApi ProductionRedisApi() {
    return RedisApi{
        ...
        [](redisContext* context, const std::vector<std::string>& arguments) {
            ...
            return static_cast<redisReply*>(redisCommandArgv(context, static_cast<int>(argv.size()),
...
class RedisConnectionPool {
  public:
    ...
    RedisConnectionPool(std::size_t pool_size, const std::string& host, int port,
        ...
        : RedisConnectionPool(
              Options{pool_size, std::chrono::seconds(5), std::chrono::seconds(60), true}, host,
              port, user, password, ProductionRedisApi()) {}
    ...
    RedisConnectionPool(Options options, std::string host, int port, std::string user,
        ...
        : options_(std::move(options)), host_(std::move(host)), port_(port), user_(std::move(user)),
          password_(std::move(password)), api_(std::move(api)) {
    ...
    redisContext* getConnection() {
        chat::observability::BorrowTimer timer(chat::observability::redis_pool_metrics);
