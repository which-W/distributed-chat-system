...
class ChatServer {
  public:
    ChatServer()
        : host(""), port(""), name(""), transport("insecure"), tls_server_name(""), con_count(0) {}
    ChatServer(const ChatServer& cs)
        : host(cs.host), port(cs.port), name(cs.name), transport(cs.transport),
          tls_server_name(cs.tls_server_name), con_count(cs.con_count) {}
