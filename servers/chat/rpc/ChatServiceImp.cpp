...
bool validTextNotification(const TextChatMsgReq& request) {
    if (request.textmsgs_size() <= 0 || request.textmsgs_size() > MAX_TEXT_MESSAGES_PER_FRAME)
        return false;
    std::size_t aggregate_bytes = 0;
    for (const auto& message : request.textmsgs()) {
        if (message.msgcontent().size() > MAX_TEXT_CONTENT_BYTES ||
            message.msgid().size() > MAX_TEXT_MESSAGE_ID_BYTES)
            return false;
...
Status ChatServiceImp::NotifyAddFriend(ServerContext* context, const AddFriendReq* request,
                                       AddFriendRsp* reply) {
    const auto auth =
        chat::internal_rpc::authorize(*context, ConfigMgr::Inst()["InternalRpc"]["PeerToken"]);
    if (!auth.ok())
        return auth;
    if (request->applyuid() <= 0 || request->touid() <= 0 || !validProfileNotification(*request) ||
...
Status ChatServiceImp::NotifyAuthFriend(ServerContext* context, const AuthFriendReq* request,
                                        AuthFriendRsp* reply) {
    const auto auth =
        chat::internal_rpc::authorize(*context, ConfigMgr::Inst()["InternalRpc"]["PeerToken"]);
    if (!auth.ok())
...
Status ChatServiceImp::NotifyTextChatMsg(::grpc::ServerContext* context,
                                         const TextChatMsgReq* request, TextChatMsgRsp* reply) {
    const auto auth =
        chat::internal_rpc::authorize(*context, ConfigMgr::Inst()["InternalRpc"]["PeerToken"]);
    if (!auth.ok())
        return auth;
    if (request->fromuid() <= 0 || request->touid() <= 0 || !validTextNotification(*request) ||
        !MysqlMgr::GetInstance()->AreFriends(request->fromuid(), request->touid())) {
    ...
    // Wake the database-backed window instead of bypassing retry/backpressure.
    if (!LogicSystem::GetInstance()->WakeDelivery(touid)) {
        reply->set_error(ErrorCodes::RPC_ERROR);
        return Status(grpc::StatusCode::RESOURCE_EXHAUSTED, "delivery wakeup queue is full");
    }
    return Status::OK;
}

bool ChatServiceImp::GetBaseInfo(std::string base_key, int uid,
...
Status ChatServiceImp::NotifyFileAvailable(::grpc::ServerContext* context,
    ...
    if (auto session = UserMgr::GetInstance()->GetSession(request->touid())) {
        ...
        value["error"] = 0;
        value["id"] = stored->id;
        value["fromuid"] = stored->sender_uid;
        value["touid"] = stored->receiver_uid;
        value["name"] = stored->original_name;
        value["mime"] = stored->mime_type;
        value["total_size"] = Json::UInt64(stored->total_size);
