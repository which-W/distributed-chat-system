...
ChatGrpcClient::ChatGrpcClient() {
    ...
    if (_auth_token.empty())
        throw std::runtime_error("InternalRpc.PeerToken is required");
    ...
    for (auto& word : words) {
        ...
        _pools[cfg[word]["Name"]] =
            std::make_unique<ChatConPool>(5, cfg[word]["Host"], cfg[word]["Port"],
                                          chat::grpc_tls::from_config(cfg), cfg[word]["TLSName"]);
    }
}
...
AddFriendRsp ChatGrpcClient::NotifyAddFriend(std::string server_ip, const AddFriendReq& req) {
    ...
    Defer defercon([&stub, this, &pool]() { pool->Return_connection(std::move(stub)); });
...
message::FileAvailableRsp
ChatGrpcClient::NotifyFileAvailable(std::string server_ip, const message::FileAvailableReq& req) {
    ...
    chat::grpc_client::setDeadline(context, chat::grpc_client::kInternalRpcTimeout);
    chat::internal_rpc::authenticate(context, _auth_token);
    if (!stub->NotifyFileAvailable(&context, req, &response).ok())
        response.set_error(ErrorCodes::RPC_ERROR);
    return response;
}
