...
void MysqlDao::EnsureChatMessageTable() {
    ...
    if (!con)
        throw std::runtime_error("chat message schema migration failed: database unavailable");
    ...
    statement->executeUpdate(
        ...
        "client_message_id VARCHAR(128) NOT NULL, sender_uid INT NOT NULL, receiver_uid INT NOT "
        "NULL, "
...
void MysqlDao::EnsureFileTransferTable() {
    auto con = pool_->getConnection();
    if (!con)
        throw std::runtime_error("file transfer schema migration failed: database unavailable");
    Defer defer([this, &con]() { pool_->returnConnection(std::move(con)); });
    std::unique_ptr<sql::Statement> statement(con->_con->createStatement());
    statement->executeUpdate(
        ...
        "KEY idx_file_receiver (receiver_uid, status, expires_at), KEY idx_file_expiry "
        "(expires_at), "
...
chat::storage::FriendAcceptanceResult MysqlDao::AddFriend(const int& from, const int& to,
    ...
    class MysqlFriendAcceptanceStore {
      public:
        explicit MysqlFriendAcceptanceStore(sql::Connection& connection)
        ...
        int acceptPendingApplication(int applicant_uid, int accepter_uid) {
            std::unique_ptr<sql::PreparedStatement> statement(
                connection_.prepareStatement("UPDATE friend_apply SET status = 1 "
                                             "WHERE from_uid = ? AND to_uid = ? AND status = 0"));
    ...
    try {
        ...
        chat::observability::stream(chat::observability::Level::Warn)
            << "Friend acceptance transaction failed: " << error.what()
...
bool MysqlDao::AreFriends(int first_uid, int second_uid) {
    return CheckFriendRelation(first_uid, second_uid).value_or(false);
}

std::optional<bool> MysqlDao::CheckFriendRelation(int first_uid, int second_uid) {
    auto con = pool_->getConnection();
    if (con == nullptr)
        return std::nullopt;
    Defer defer([this, &con]() { pool_->returnConnection(std::move(con)); });
    try {
        ...
        chat::observability::stream(chat::observability::Level::Warn)
            << "Friend authorization query failed: " << error.what() << std::endl;
        return std::nullopt;
    }
}
...
bool MysqlDao::HasPendingFriendApply(int from_uid, int to_uid) {
    ...
    try {
        ...
        chat::observability::stream(chat::observability::Level::Warn)
            << "Friend application authorization query failed: " << error.what() << std::endl;
        return false;
    }
}
...
bool MysqlDao::PersistTextMessages(const std::vector<chat::messages::TextMessage>& messages) {
    ...
    try {
        ...
        std::unique_ptr<sql::PreparedStatement> verify(
            con->_con->prepareStatement("SELECT receiver_uid,content FROM chat_message "
        ...
        chat::observability::stream(chat::observability::Level::Warn)
            << "Persist text messages failed: " << error.what() << std::endl;
        return false;
    }
}

std::optional<std::vector<chat::messages::TextMessage>>
MysqlDao::GetPendingTextMessages(int receiver_uid, int limit) {
    std::vector<chat::messages::TextMessage> messages;
    if (receiver_uid <= 0 || limit <= 0)
        return std::nullopt;
    auto con = pool_->getConnection();
    if (!con)
        return std::nullopt;
    ...
    try {
        ...
        chat::observability::stream(chat::observability::Level::Warn)
            << "List pending text messages failed: " << error.what() << std::endl;
        return std::nullopt;
    }
    return messages;
}
...
bool MysqlDao::AcknowledgeTextMessages(int receiver_uid, int sender_uid,
    ...
    if (receiver_uid <= 0 || sender_uid <= 0 || client_message_ids.empty())
        return false;
    auto con = pool_->getConnection();
    if (!con)
        return false;
    ...
    try {
        ...
        chat::observability::stream(chat::observability::Level::Warn)
            << "Acknowledge text messages failed: " << error.what() << std::endl;
        return false;
    }
}
...
bool MysqlDao::CreateFileTransfer(const chat::files::TransferRecord& transfer) {
    ...
    try {
        ...
        std::unique_ptr<sql::PreparedStatement> sender_lock(
            con->_con->prepareStatement("SELECT uid FROM user WHERE uid=? FOR UPDATE"));
        ...
        std::unique_ptr<sql::PreparedStatement> quota(
            con->_con->prepareStatement("SELECT COUNT(*) AS item_count,COALESCE(SUM(total_size),0) "
                                        "AS total_bytes FROM file_transfer "
                                        "WHERE sender_uid=? AND expires_at>NOW() AND status "
                                        "IN('uploading','available','downloaded')"));
        ...
        std::unique_ptr<sql::PreparedStatement> statement(con->_con->prepareStatement(
            "INSERT INTO "
            "file_transfer(id,sender_uid,receiver_uid,original_name,mime_type,total_size,"
            "uploaded_size,sha256,status,expires_at) VALUES(?,?,?,?,?,?,0,?,'uploading',"
            "DATE_ADD(NOW(), INTERVAL 7 DAY))"));
        statement->setString(1, transfer.id);
        statement->setInt(2, transfer.sender_uid);
        statement->setInt(3, transfer.receiver_uid);
        statement->setString(4, transfer.original_name);
        statement->setString(5, transfer.mime_type);
        statement->setUInt64(6, transfer.total_size);
        ...
        chat::observability::stream(chat::observability::Level::Warn)
            << "Create file transfer failed: " << error.what() << std::endl;
        return false;
    }
}
...
std::optional<chat::files::TransferRecord> MysqlDao::GetFileTransfer(const std::string& id) {
    ...
    try {
        std::unique_ptr<sql::PreparedStatement> statement(
            con->_con->prepareStatement("SELECT "
                                        "id,sender_uid,receiver_uid,original_name,mime_type,total_"
                                        "size,uploaded_size,sha256,status "
        ...
        record.sender_uid = row->getInt("sender_uid");
        record.receiver_uid = row->getInt("receiver_uid");
        record.original_name = row->getString("original_name");
        record.mime_type = row->getString("mime_type");
        record.total_size = row->getUInt64("total_size");
        record.uploaded_size = row->getUInt64("uploaded_size");
        record.sha256 = row->getString("sha256");
        ...
        record.status = status == "uploading"    ? chat::files::TransferStatus::Uploading
                        ...
                        : status == "cancelled"  ? chat::files::TransferStatus::Cancelled
                                                 : chat::files::TransferStatus::Expired;
        return record;
    } catch (const sql::SQLException& error) {
        chat::observability::stream(chat::observability::Level::Warn)
            << "Get file transfer failed: " << error.what() << std::endl;
        return std::nullopt;
    }
}
...
std::vector<chat::files::TransferRecord> MysqlDao::GetPendingFileTransfers(int receiver_uid) {
    ...
    try {
        ...
        while (rows->next())
            ids.push_back(rows->getString("id"));
        rows.reset();
        statement.reset();
        for (const auto& id : ids)
            if (auto item = GetFileTransfer(id))
                records.push_back(*item);
    } catch (const sql::SQLException& error) {
        chat::observability::stream(chat::observability::Level::Warn)
            << "List file transfers failed: " << error.what() << std::endl;
    }
    return records;
}
...
bool MysqlDao::AdvanceFileUpload(const std::string& id, int sender_uid,
    ...
    Defer defer([this, &con]() { pool_->returnConnection(std::move(con)); });
    try {
        std::unique_ptr<sql::PreparedStatement> s(con->_con->prepareStatement(
            "UPDATE file_transfer SET uploaded_size=? WHERE id=? AND sender_uid=? AND "
            "status='uploading' "
            "AND uploaded_size=? AND ?>uploaded_size AND ?<=total_size"));
        s->setUInt64(1, new_offset);
        s->setString(2, id);
        s->setInt(3, sender_uid);
        s->setUInt64(4, expected_offset);
        s->setUInt64(5, new_offset);
        s->setUInt64(6, new_offset);
...
bool MysqlDao::CompleteFileTransfer(const std::string& id, int sender_uid) {
    ...
    try {
        std::unique_ptr<sql::PreparedStatement> s(con->_con->prepareStatement(
            "UPDATE file_transfer SET "
            "status='available',completed_at=NOW(),expires_at=DATE_ADD(NOW(),INTERVAL 7 DAY) "
            "WHERE id=? AND sender_uid=? AND status='uploading' AND uploaded_size=total_size"));
        s->setString(1, id);
        s->setInt(2, sender_uid);
        return s->executeUpdate() == 1;
    } catch (const sql::SQLException&) {
        return false;
    }
...
bool MysqlDao::MarkFileDownloaded(const std::string& id, int receiver_uid) {
    ...
    try {
        std::unique_ptr<sql::PreparedStatement> s(con->_con->prepareStatement(
            "UPDATE file_transfer SET "
            "status='downloaded',downloaded_at=COALESCE(downloaded_at,NOW()),"
            "expires_at=LEAST(expires_at,DATE_ADD(NOW(),INTERVAL 1 DAY)) WHERE id=? AND "
            "receiver_uid=? "
            "AND status IN('available','downloaded')"));
        s->setString(1, id);
        s->setInt(2, receiver_uid);
        return s->executeUpdate() == 1;
    } catch (const sql::SQLException&) {
        return false;
    }
...
bool MysqlDao::CancelFileTransfer(const std::string& id, int actor_uid) {
    ...
    try {
        std::unique_ptr<sql::PreparedStatement> s(con->_con->prepareStatement(
            "UPDATE file_transfer SET status='cancelled',expires_at=NOW() WHERE id=? AND "
            "(sender_uid=? OR receiver_uid=?) "
            "AND status IN('uploading','available','downloaded')"));
        s->setString(1, id);
        s->setInt(2, actor_uid);
        s->setInt(3, actor_uid);
        return s->executeUpdate() == 1;
    } catch (const sql::SQLException&) {
        return false;
    }
...
std::vector<std::string> MysqlDao::GetExpiredFileTransferIds() {
    ...
    Defer defer([this, &con]() { pool_->returnConnection(std::move(con)); });
    try {
        std::unique_ptr<sql::Statement> s(con->_con->createStatement());
        std::unique_ptr<sql::ResultSet> rows(
            s->executeQuery("SELECT id FROM file_transfer WHERE expires_at<=NOW() OR status "
                            "IN('cancelled','expired') LIMIT 200"));
        while (rows->next())
            ids.push_back(rows->getString("id"));
    } catch (const sql::SQLException&) {
    }
...
bool MysqlDao::DeleteFileTransfer(const std::string& id) {
    ...
    Defer defer([this, &con]() { pool_->returnConnection(std::move(con)); });
    try {
        std::unique_ptr<sql::PreparedStatement> s(
            con->_con->prepareStatement("DELETE FROM file_transfer WHERE id=? AND "
                                        "(expires_at<=NOW() OR status IN('cancelled','expired'))"));
        s->setString(1, id);
        return s->executeUpdate() == 1;
    } catch (const sql::SQLException&) {
        return false;
