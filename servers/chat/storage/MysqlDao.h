...
class MysqlDao {
  public:
    ...
    chat::storage::FriendAcceptanceResult AddFriend(const int& from, const int& to,
                                                    std::string back_name);
    std::shared_ptr<UserInfo> GetUser(int uid);
    std::shared_ptr<UserInfo> GetUser(std::string name);
    bool GetApplyList(int touid, std::vector<std::shared_ptr<ApplyInfo>>& applyList, int offset,
                      int limit);
    bool GetFriendList(int self_id, std::vector<std::shared_ptr<UserInfo>>& user_info);
    // 私聊授权必须由服务端查询双方好友关系，不能依赖客户端好友列表。
    bool AreFriends(int first_uid, int second_uid);
    std::optional<bool> CheckFriendRelation(int first_uid, int second_uid);
    ...
    std::optional<std::vector<chat::messages::TextMessage>> GetPendingTextMessages(int receiver_uid,
                                                                                   int limit = 200);
    ...
    bool AdvanceFileUpload(const std::string& id, int sender_uid, std::uint64_t expected_offset,
