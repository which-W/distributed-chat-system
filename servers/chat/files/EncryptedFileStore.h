...
class EncryptedFileStore : public chat::resources::ResourceStore {
  public:
    ...
    std::vector<unsigned char> read(const std::string& transfer_id, std::uint64_t offset,
  ...
  private:
    ...
    static std::array<unsigned char, 24> nonceFor(const std::array<unsigned char, 16>& prefix,
