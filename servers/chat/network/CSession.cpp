...
CSession::CSession(boost::asio::io_context& io_context, CServer* server)
    : _socket(boost::asio::make_strand(io_context)), _server(server), _b_close(false),
      _b_head_parse(false), _user_uid(0), _auth_timer(_socket.get_executor()),
      _idle_timer(_socket.get_executor()) {
...
CSession::~CSession() {
    chat::observability::log(chat::observability::Level::Debug, "session.destroyed",
                             "chat session destroyed",
                             {{"session_id", _session_id}, {"uid", std::int64_t(GetUserId())}});
}
...
void CSession::Start() {
    auto self = shared_from_this();
    boost::asio::dispatch(_socket.get_executor(), [self] { self->StartOnExecutor(); });
}

void CSession::StartOnExecutor() {
    if (IsClosed())
        return;
    ...
    _auth_timer.async_wait([self](const boost::system::error_code& error) {
        if (!error && self->GetUserId() <= 0) {
            chat::observability::log(chat::observability::Level::Warn, "session.auth_timeout",
                                     "chat session authentication timed out",
...
bool CSession::Send(std::string msg, short msgid) {
    ...
    const auto frame_bytes = msg.size() + HEAD_TOTAL_LEN;
    if (send_que_size >= MAX_SENDQUE || frame_bytes > SendByteLimit - _send_bytes) {
        chat::observability::log(chat::observability::Level::Warn, "queue.full",
                                 "session send queue is full",
                                 {{"session_id", _session_id},
                                  {"uid", std::int64_t(GetUserId())},
                                  {"queue_depth", std::int64_t(send_que_size)},
                                  {"queue_bytes", std::uint64_t(_send_bytes)}});
        return false;
    }

    _send_que.push(make_shared<SendNode>(msg.c_str(), msg.length(), msgid));
    _send_bytes += frame_bytes;
    if (send_que_size > 0) {
        return true;
    }
    auto self = shared_from_this();
    boost::asio::post(_socket.get_executor(), [self] { self->StartWrite(); });
    return true;
}
...
bool CSession::Send(char* msg, std::size_t max_length, short msgid) {
    ...
    return Send(std::string(msg, max_length), msgid);
}
...
void CSession::StartWrite() {
    ...
    if (_send_que.empty())
        return;
    const auto& node = _send_que.front();
    boost::asio::async_write(
        _socket, boost::asio::buffer(node->_data, node->_total_len),
    ...
    _idle_timer.async_wait([self](const boost::system::error_code& error) {
        if (!error) {
            chat::observability::log(
                chat::observability::Level::Info, "session.idle_timeout",
                "chat session idle timeout",
                {{"session_id", self->_session_id}, {"uid", std::int64_t(self->GetUserId())}});
            self->Close();
        }
    });
}
...
void CSession::AsyncReadBody(int total_len) {
    ...
    asyncReadFull(total_len, [self, this, total_len](const boost::system::error_code& ec,
        ...
        try {
            if (ec) {
                chat::observability::log(chat::observability::Level::Debug, "tcp.read_failed",
                                         "chat body read failed",
                                         {{"session_id", _session_id}, {"error", ec.message()}});
                Close();
                return;
            }
            ...
            if (bytes_transfered < total_len) {
                chat::observability::stream(chat::observability::Level::Info)
                    << "read length not match, read [" << bytes_transfered << "] , total ["
                    << total_len << "]" << endl;
                Close();
                return;
            }
...
void CSession::AsyncReadHead(int total_len) {
    ...
    asyncReadFull(HEAD_TOTAL_LEN, [self, this](const boost::system::error_code& ec,
        ...
        try {
            if (ec) {
                chat::observability::stream(chat::observability::Level::Info)
                    << "handle read failed, error is " << ec.what() << endl;
                Close();
                return;
            }
            ...
            if (!IsClientRequestMessage(msg_id)) {
                chat::observability::log(
                    chat::observability::Level::Warn, "protocol.invalid_message_id",
                    "invalid client message id",
                    {{"session_id", _session_id}, {"msg_id", std::uint64_t(msg_id)}});
                Close();
                return;
            }
            ...
            if (msg_len == 0 || msg_len > allowed_length) {
                chat::observability::log(chat::observability::Level::Warn,
                                         ...
                                         {{"session_id", _session_id},
                                          {"msg_id", std::uint64_t(msg_id)},
                                          {"bytes", std::uint64_t(msg_len)}});
                Close();
                return;
            }
...
void CSession::HandleWrite(const boost::system::error_code& error,
    ...
    try {
        ...
        if (!error) {
            std::lock_guard<std::mutex> lock(_send_lock);
            // chat::observability::stream(chat::observability::Level::Info) << "send data " <<
            // _send_que.front()->_data+HEAD_LENGTH << endl;
            _send_bytes -= _send_que.front()->_total_len;
            _send_que.pop();
            if (!_send_que.empty() && !IsClosed()) {
            ...
            chat::observability::log(chat::observability::Level::Debug, "tcp.write_failed",
                                     "chat write failed",
                                     {{"session_id", _session_id}, {"error", error.message()}});
            Close();
        }
    } catch (std::exception& e) {
        chat::observability::stream(chat::observability::Level::Warn)
            << "Exception code : " << e.what() << endl;
    }
}
...
//	if (diff_sec > 20) {
//		chat::observability::stream(chat::observability::Level::Info) << "heartbeat expired,
// session id is  " << _session_id << endl; 		return true;
//	}
//
//	return false;
// }
...
//	auto identifier = RedisMgr::GetInstance()->acquireLock(lock_key, LOCK_TIME_OUT,
// ACQUIRE_TIME_OUT); 	Defer defer([identifier, lock_key, self, this]() {
//		_server->ClearSession(_session_id);
...
// redis_session_id); 	if (!bsuccess) { 		return;
//	}
//
