...
chat::storage::RedisApi MakeApi(FakeRedisState& state) {
    return {[&state](const std::string&, int) {
                ++state.connected;
            ...
            [&state](redisContext*, const std::vector<std::string>& arguments) -> redisReply* {
                ...
                if (!arguments.empty() && arguments.front() == "AUTH") {
                    if (state.auth_mode == FakeRedisState::AuthMode::NullReply)
                        return nullptr;
...
int main() {
    {
        ...
        chat::storage::RedisConnectionPool pool(Options(), "127.0.0.1", 6379, "", "",
                                                MakeApi(state));
        if (!Require(FirstCommand(state).empty(), "空密码不应发送 AUTH"))
            return 1;
    }
    {
        FakeRedisState state;
        chat::storage::RedisConnectionPool pool(Options(), "127.0.0.1", 6379, "", "secret",
                                                MakeApi(state));
    ...
    {
        FakeRedisState state;
        chat::storage::RedisConnectionPool pool(Options(), "127.0.0.1", 6379, "chat", "secret",
                                                MakeApi(state));
        if (!Require(FirstCommand(state) == std::vector<std::string>{"AUTH", "chat", "secret"},
                     "ACL 认证参数错误"))
            return 1;
    }
    for (const auto mode :
         {FakeRedisState::AuthMode::NullReply, FakeRedisState::AuthMode::ErrorReply}) {
        FakeRedisState state;
        state.auth_mode = mode;
        chat::storage::RedisConnectionPool pool(Options(), "127.0.0.1", 6379, "chat", "bad",
                                                MakeApi(state));
    ...
    }
    {
        ...
        chat::storage::RedisConnectionPool pool(Options(true), "127.0.0.1", 6379, "chat", "secret",
                                                MakeApi(state));
        std::this_thread::sleep_for(std::chrono::milliseconds(80));
        if (!Require(state.connected >= 2, "PING 失败后未重建 Redis 连接"))
            return 1;
        auto* recovered = pool.getConnection();
        if (!Require(recovered != nullptr, "Redis 重连后未恢复池容量"))
            return 1;
    ...
    }
    {
        ...
        if (!Require(state.freed_contexts == 1, "Close 后归还的 Redis 连接未释放"))
            return 1;
    }
    return 0;
}
