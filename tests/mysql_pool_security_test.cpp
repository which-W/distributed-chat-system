...
int main() {
    ...
    if (!Require(native.auto_commit, "事务清理后未恢复 autocommit") ||
        !Require(native.calls ==
                     std::vector<std::string>{"getAutoCommit", "rollback", "setAutoCommit"},
                 "事务清理调用顺序错误")) {
        return 1;
    }

    {
        int next_id = 0;
        MySqlPool pool(
            TestOptions(1), [&]() { return std::make_unique<SqlConnection>(nullptr, ++next_id); },
            [](SqlConnection&) {}, [](SqlConnection&) {});
        auto first = pool.getConnection();
        if (!Require(first && first->_last_oper_time == 1, "未借到初始假连接"))
            return 1;
        pool.returnConnection(std::move(first));
        auto reused = pool.getConnection();
        if (!Require(reused && reused->_last_oper_time == 1, "健康连接没有重新入池"))
            return 1;
...

    {
        ...
        MySqlPool pool(
            ...
            [](SqlConnection& connection) {
                if (connection._last_oper_time == 1)
                    throw std::runtime_error("dirty");
            },
            [](SqlConnection&) {});
...

    {
        ...
        MySqlPool pool(
            ...
            [&]() -> std::unique_ptr<SqlConnection> {
                ...
                if (call == 2)
                    return nullptr; // 同步补建失败，后台线程必须继续恢复。
...

    {
        ...
        MySqlPool empty(
            options, []() { return std::unique_ptr<SqlConnection>(); }, [](SqlConnection&) {},
            [](SqlConnection&) {});
        const auto started = std::chrono::steady_clock::now();
        if (!Require(!empty.getConnection(), "空池不应返回连接"))
            return 1;
        const auto elapsed = std::chrono::steady_clock::now() - started;
        if (!Require(elapsed < std::chrono::milliseconds(250), "空池等待没有按时结束"))
            return 1;
    }
